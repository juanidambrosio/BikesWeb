...
namespace BikeSharing.Client.Kiosk.UWP.STT
{
    public class SpeechToText : IDisposable
    {
        ...
        public async Task<bool> InitializeRecognizerAsync()
        {
            ...
            {
                //TODO (yumeng): demo-only, in case of network issue
                string[] responses =
                {
        ...
        private async Task<bool> IsOffline()
        {
            try
            {
                var client = new HttpClient();
                client.Timeout = TimeSpan.FromSeconds(3);
                var result = await (client.GetAsync("http://www.bing.com"));
            }
            catch (Exception ex)
            {
                Debug.WriteLine("[Speech to Text]: offline mode");
                return true;
            }
        ...
        public async Task<string> GetTextFromSpeechAsync(bool withUI)
        {
            if (_recognizer == null)
            {
